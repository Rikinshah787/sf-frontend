...
describe("addressLine", () => {
  const address = (overrides: Partial<Parameters<typeof addressLine>[0]> = {}) => ({
    type: "home" as const,
    address: null,
    city: "San Francisco",
    state: "CA",
  ...
  it("pairs the state with the postal code", () => {
    expect(
      addressLine(address({ address: "1 Market St", postal_code: "94105" })),
    ).toBe("1 Market St, San Francisco, CA 94105, USA");
  });

  it("returns null when there is no address at all", () => {
    expect(
      addressLine(address({ city: null, state: null, country: null })),
    ).toBeNull();
  });
});
